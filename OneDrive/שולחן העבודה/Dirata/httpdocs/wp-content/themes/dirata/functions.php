...
function enqueue_mapbox_scripts() {
    ...
    wp_enqueue_script('custom-mapbox', get_template_directory_uri() . '/assets/js/mapbox-init.js', ['mapbox-js'], null, true);
    ...
    foreach ($users as $user) {
        ...
        foreach (['daily', 'every_week'] as $cron_type) {
            ...
            while ($query->have_posts()) {
                ...
                <body style='font-family: Oscarv3FM-Regular, sans-serif; background: #f9f9f9; padding: 30px; text-align: center;'>
                <img src='{$logo_url}' alt='דירתא' style='max-width: 200px; margin-bottom: 30px;'>

                  <img src='{$main_img}' alt='' style='width: 100%; max-width: 600px; border-radius: 8px;'>
            
                  <h2 style='color: #333;'>{$sale_label}</h2>
                  <h1 style='color: #333;'>" . get_the_title() . "</h1>
                  <h3 style='color: #555;'>{$price_label}</h3>
            
                  <p style='color: #777;'>
                      עיר: {$city} &nbsp; | &nbsp; אזור: {$area}
                  </p>
            
                  <a href='{$apartment_url}' style='display: inline-block; margin: 15px; padding: 15px 30px; background: #4CAF50; color: #fff; text-decoration: none; border-radius: 5px;'>
                      מעבר לפרטים נוספים על הדירה
                  </a>
            
                  <a href='{$profile_url}' style='display: inline-block; margin: 15px; padding: 15px 30px; background: #003366; color: #fff; text-decoration: none; border-radius: 5px;'>
                      מעבר לאזור האישי
                  </a>
