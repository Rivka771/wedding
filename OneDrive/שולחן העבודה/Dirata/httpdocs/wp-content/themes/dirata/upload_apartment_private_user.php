<?php
/**
 * Template Name: Upload Apartment Private User
 */

 if ( ! is_user_logged_in() ) {
    wp_redirect( 'https://dirata.co.il/private-signup/?redirect_to=' . urlencode( get_permalink() ) );
    exit;
}
// get_header();
...
<div class="private_apartment_form_wrapper">
  <div class="container">
    <div class="row no-gutters">
      <div class="col-lg-7 private_apartment_form_order1">
      ...
      <!-- הבאנר בצד בעלות 150₪ -->
      <div class="col-lg-5 private_apartment_form_order2 private_apartment_form_ab"
          style="background-image: url('<?php echo get_template_directory_uri(); ?>/assets/img/bunner_upload_ap.png'); background-size: cover; background-position: center; background-repeat: no-repeat; height: 100%;">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const submitBtn = form.querySelector('.detail_payment_btn');
      ...
      form.addEventListener('submit', function (e) {
        ...
        submitBtn.innerHTML = 'אנחנו מעבדים את נתוני הדירה שלך... ⏳';
        ...
        fetch(dirataVars.ajaxurl, {
          ...
          .then(response => {
            ...
            if (response.success) {
              if (response.data.redirect_url) {
                window.location.href = response.data.redirect_url; // 🔄 מפנה לתקבול
              } else {
                // מתווך – ניהול רגיל
                form.innerHTML = `
              ...
              alert(response.data.message || 'משהו השתבש, נסה שוב.');
            }
          })
          .catch(error => {
            resetSubmitBtn();
          ...
          function resetSubmitBtn() {
            ...
            submitBtn.innerHTML = 'לפרסום הדירה למכירה ←';
          }
